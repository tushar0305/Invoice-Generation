    ...
    return (
        ...
        >
            <div className="container px-4 md:px-6 mx-auto">
                        ...
                        <CarouselContent className="-ml-4">
                            {plans.map((plan, index) => (
                                <CarouselItem key={plan.name} className="pl-4 basis-[85%]">
                                    <PricingCard plan={plan} index={index} shouldReduceMotion={shouldReduceMotion} handleGetStarted={handleGetStarted} />
                                </CarouselItem>
                            ))}
                        </CarouselContent>
                    </Carousel>
                ...
                <div
                    className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto"
                    role="list"
                    aria-label="Pricing plans"
                >
                    {plans.map((plan, index) => (
                        <PricingCard key={plan.name} plan={plan} index={index} shouldReduceMotion={shouldReduceMotion} handleGetStarted={handleGetStarted} />
                    ))}
                </div>
            </div>
        </section>
...
function PricingCard({ plan, index, shouldReduceMotion, handleGetStarted }: { plan: Plan, index: number, shouldReduceMotion: boolean | null, handleGetStarted: (name: string) => void }) {
    return (
        <motion.article
            ...
            className={`relative p-6 md:p-8 rounded-2xl border flex flex-col text-center h-full ${plan.popular
