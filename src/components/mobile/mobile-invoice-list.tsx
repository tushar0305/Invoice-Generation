...
import { Search, FileText, Calendar, CheckCircle2, Clock, AlertCircle, Plus } from 'lucide-react';
...
export function MobileInvoiceList({ shopId, invoices }: MobileInvoiceListProps) {
  ...
  const filteredInvoices = invoices.filter(inv => {
    const matchesSearch = (inv.customerSnapshot?.name || '').toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div className="md:hidden min-h-screen bg-gray-50/50 dark:bg-black/50 pb-24">
                ...
                <Card className="border-none shadow-sm bg-white dark:bg-gray-900/50 active:scale-[0.98] transition-transform duration-200">
                  <CardContent className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h3 className="font-semibold text-gray-900 dark:text-gray-100">{inv.customerSnapshot?.name || 'Unknown'}</h3>
                        <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                          <Calendar className="w-3 h-3" />
                          {format(new Date(inv.invoiceDate), 'dd MMM yyyy')}
                        </p>
                      </div>
                      <div className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide flex items-center gap-1 ${inv.status === 'paid'
      ...
      <Link href={`/shop/${shopId}/invoices/new`}>
        <Button
          size="lg"
          className="fixed bottom-24 right-4 h-14 w-14 rounded-full shadow-lg shadow-primary/30 bg-primary hover:bg-primary/90 z-40"
        >
